	...
	for _, tt:= range tests{
		...
		t.Run(tt.name, func(t *testing.T){
			...
			if tt.wantErr && !errors.Is(err, tt.errType) {
				t.Fatalf("NewUser() error=%v, but wantErrType %v", err, tt.errType)
			}
...
func TestUser_UpdateUser(t *testing.T){
	...
	for _, tt := range tests{
		...
		t.Run(tt.name, func(t *testing.t){
			...
			if diff := cmp.Diff(got, tt.want, cmp.AllowUnexported(User{}, Email{}, HashedPassword{}), cmpopts.IgnoreFields(User{}, "id", "hashPassword")); diff != ""{
...
func TestComparePassword(t *testing.T)
{
	...
	{
		{
			name: "正常系: パスワードの比較で成功する",
			plain: "password",
			wantErr: false,
		},
		{
			name: "準正常系: パスワードの比較で失敗する",
			plain: "incorrect",
			wantErr: true,
		}
	}
	for _, tt := range tests{
		...
		t.Run(tt.name, func(t *testing.T){
			...
			if tt.wantErr && !errors.Is(err, errors.ErrPasswordMismatch){
				t.Fatalf("ComparePassword() expected ErrPasswordMismatch, got: %v", err)
			}
		})
	}
}