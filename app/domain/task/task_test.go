...
import (
	"testing"
	"github.com/git-practice/go-cmp/cmp"
	"github.com/git-practice/go-cmp/cmp/cmpopts"
...
func TestNewTask(t *testing.T){
	...
	for _, tt := range tests{
		...
		t.Run(tt.name, func(t *testing.T){
			...
			if tt.wantErr && !error.Is(err, tt.errType){
				t.Fatalf("NewTask() error=%v, but wantErrType %v", err, tt.wantErrType)
			}
	...
	for _, tt := range tests{
		...
		t.Run(tt.name, func(t *testing.T){
			...
			if(tt.wantErr) != errors.Is(err, tt.errType){
				t.Fatalf("UpdateState() error=%v, wantErrType=%v", err, tt.wantErrType)
			}
			if diff := cmp.Diff(got, tt.want, cmpopts.IgnoreFields(Task{}, "id"), cmp.AllowUnexported(Task{}, Content{})); diff !=""{
